...
class BaseAgentExecutor(AgentExecutor, ABC):
    ...
    async def run_astream_in_agent_server(self, server_agent:ServerAgentRequest,  client : TaskUpdater | None = None):
        ...
        async for mode, payload in self.agent.agent.astream(
            server_agent.input_payload.data if  server_agent.input_payload.type =="input_text" else Command(resume=server_agent.input_payload.data),
            config=config,
            ...
            if "__interrupt__" in payload:
                await client.update_status(
                        ...
                        message=new_agent_text_message(dict_to_string(payload),server_agent.context_id, server_agent.task_id)
                    )
