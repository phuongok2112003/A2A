...
def create_deep_agent(
    ...
    models: dict[str, str | BaseChatModel | None] | None = None,
    ...
    *,
    ...
    if models is not None:
        ...
        summary_model = models.get('summary_model')

        if summary_model is not None and isinstance(summary_model, str):
            summary_model = init_chat_model(summary_model)

        if primary is not None:
            if isinstance(primary, str):
                model = init_chat_model(primary)
            else:
                model = primary
