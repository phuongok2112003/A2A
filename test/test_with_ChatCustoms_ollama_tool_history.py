...
def searxng_search(
    ...
    params: dict[str, str | int] = {"q": keyword, "format": "json"}
    if time_range:
...
def search_web(query: str, max_results: int = 5, time_range: str = "day") -> list[dict[str, str]]:
    """
    ...
    Args:
        ...
        time_range: SearXNG time range filter. Use "day" for today's news,
            "week" for recent weekly news, or "" to disable date filtering.
    """
    normalized_time_range = time_range or None
    return searxng_search(keyword=query, limit=max_results, time_range=normalized_time_range)
...
def fetch_webpage_text(url: str) -> dict[str, str]:
    ...
    try:
        ...
        if sync_playwright is None:
            return {
                ...
                "title": "Fetch failed",
                "content": (
                    "Unable to fetch this webpage with HTTP extraction, and "
                    f"Playwright is unavailable. Error: {error}"
                ),
                "error": str(error),
            }
    ...
    try:
        ...
        return {
            ...
            "title": "Fetch failed",
            "content": (
                "Unable to fetch this webpage. The agent should skip this source "
                "or rely on the search snippet and other fetched pages.\n"
...
def get_current_datetime() -> dict[str, str]:
    """
    Return the current time in UTC and Vietnam time so the agent can compare
    event dates against "today" before deciding whether an event is current.

    Use this tool before time-sensitive searches. Use current_vietnam_date when
    the user asks for "hôm nay" or today's news in Vietnamese.
    """
    now_utc = datetime.now(timezone.utc)
    now_vietnam = now_utc.astimezone(ZoneInfo("Asia/Ho_Chi_Minh"))
    return {
        "current_utc_iso": now_utc.isoformat(),
        "current_utc_date": now_utc.strftime("%Y-%m-%d"),
        "current_utc_datetime": now_utc.strftime("%Y-%m-%d %H:%M:%S UTC"),
        "current_vietnam_iso": now_vietnam.isoformat(),
        "current_vietnam_date": now_vietnam.strftime("%Y-%m-%d"),
        "current_vietnam_datetime": now_vietnam.strftime("%Y-%m-%d %H:%M:%S %Z"),
    }


async def main() -> None:
    prompt = (
        "Tổng hợp thông tin mới nhất về AI trong ngày hôm nay theo giờ Việt Nam. "
        "Hãy tự gọi get_current_datetime để lấy ngày hiện tại, dùng current_vietnam_date "
        "đó để tạo truy vấn search_web với time_range=\"day\". Chỉ đưa tin được xuất "
        "bản/cập nhật trong đúng ngày hôm nay hoặc snippet ghi rõ trong vài giờ qua. "
        "Bỏ qua trang category, trang hub, bài cũ, hoặc nguồn không có ngày rõ ràng. "
        "Nếu không xác minh được là tin hôm nay, hãy nói rõ là chưa xác minh thay vì "
        "đưa vào như tin mới."
    )
    llm = ChatCustoms(
